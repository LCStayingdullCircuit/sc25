...
{  
    ...
    int Iternumber = 0;  
    std::vector<double> normsRatios;  
    int flag = Solve<double>(cublas_handle, dtA, m, n, d_b, d_x, shift, tol, solvermaxit, quiet, normsRatios);  

    cudaMemcpy(d_b, b.data(), sizeof(double) * m, cudaMemcpyHostToDevice);  
    cudaMemcpy(d_x, x.data(), sizeof(double) * n, cudaMemcpyHostToDevice);  

    flag = cgSolve<double>(cublas_handle, dtA, dR, m, n, d_b, d_x, shift, tol, cgsovlermaxit, quiet, Iternumber);  
    cudaDeviceSynchronize();  
